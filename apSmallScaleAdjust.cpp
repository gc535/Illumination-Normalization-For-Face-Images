...
unsigned char apFindThreshold(IplImage *src)
{
	...
	char *dst;
	dst = (char *)calloc(k, sizeof(unsigned char));
	apCopyMatrix1Duc((unsigned char *)(src->imageData), (unsigned char *)dst, k);
	...
	apReleaseMatrix1Duc((unsigned char *)dst);
	return threshold;
}
...
char apFindThreshold_data(char *imageData, int size)
{
	...
	apCopyMatrix1Duc((unsigned char *)(imageData), (unsigned char *)dst, k);
